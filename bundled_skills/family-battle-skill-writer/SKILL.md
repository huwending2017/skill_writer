...
- For display-only stack buffs, `overlying` is only a presentation carrier. Keep the authoritative business stack count in the core buff runtime or in a deliberate state-provider buff. A `BUFF_OVERLYING_EFFECT_FUNC` callback must not copy display-buff `overlying` back into the real runtime state, because display buffs may be cleared or rebuilt with `overlying = 0` during refresh, temporary invalidation, or removal flows.
- War report IDs must not be treated as tunable buff parameters. Do not place literal war-report IDs in `buff.param`, `skill_stage.param`, or other Excel config parameter lists. If a production buff/action needs to insert a war report, resolve it inside that Lua script through the war-report config enum/constant/name used by the battle runtime, and document the insertion point in comments.
- For resident buff/event-triggered war reports, `make_effect_records(...)` alone is not enough. If the buff is reacting to another skill or damage event, write records through the current `extern.skill` flow: clean the resident buff's temporary effect list, add the buff word if appropriate, call `make_effect_records(...)`, then `insert_effect_list(extern.skill, nil)`. Otherwise debug logs may show the record was prepared while the front-end battle report never displays it.
- Treat battle reports as part of the mechanic contract, not as decoration. Before implementation, build a war-report coverage matrix from the design and include it in `IMPLEMENTATION.md`: trigger entry, probability success/fail, stack gain/loss/current value, threshold conversion, damage/cure/control/dispel result, state add/remove/expire, temporary invalid/restore, first-trigger versus later-trigger, cap/no-target/no-effect branches, and any displayed value that differs from internal runtime state.
- Every state mutation that a player must understand should either have an existing runtime report or an explicit custom `war_paper` row. This includes "gained stack", "lost stack", "converted stack", "damage reduction amount", "attribute/suck/other per-stack benefit", "dispel attempted/succeeded/no target", "first threshold reached", "state starts", "state expires", and "effect temporarily invalid/continues".
- When adding a custom battle report, define its placeholders from the real values inserted by Lua. Use `RECORD_NUM_DEF.PERCENT_TYPE` for percentage values and do not add manual `%%` in the war-paper text unless the runtime format requires a literal percent sign for a non-percent placeholder. The report text, `num_list` order, and `data_war_paper.<name>` mapping must be reviewed together.
- For event-triggered/resident-buff reports, verify ordering against the actual front-end battle report, not only DEBUG logs. Reports produced during damage/cure chains often need to be inserted into the original trigger skill after the engine's damage record has been created; do not move them to round end or action end unless the design explicitly wants that.
- New `war_paper` payload rows should leave `ID` empty unless they intentionally update an existing row. The Excel writeback tool owns final ID assignment and should append from the current maximum ID in `Z_战报表.xlsx`; do not invent large placeholder IDs in payloads or Lua.
- New `war_paper` payload rows must use a readable ASCII Lua enum key in `name` such as `yuefei_report_country_gain` or `shen_sheng_zhi_zhe_trigger`; never put Chinese text, spaces, punctuation, or pure hash-like names in `name`.
- New `war_paper` payload rows must put the visible Chinese report text in `desc1`. Do not output only `desc`; the tool has a compatibility fallback, but skill generation should always use the real Excel field name.
...
- Function-level comments are not enough. Key in-function branches must also have nearby Chinese comments, especially non-obvious `if/return`, stack mutation, cache write/read, listener registration, temporary invalidation recovery, war-report cleanup, damage mutation, and state-provider consumption.
...
- Detailed DEBUG logging is mandatory for new or materially changed production scripts. Use direct `DEBUG("[技能名]", ...)` calls at the real logic branch, not a shared `debug_log(...)` wrapper, so battle logs show the exact Lua line that made the decision. Log init/uninit, event dispatch, probability/threshold decisions, state changes, stack changes, target counts, damage/cure values, war-report IDs, and early returns that commonly explain "why nothing happened".
- The local artifact audit must fail generated production Lua when meaningful functions lack nearby Chinese comments, when the script lacks parameter/event/state/exception/report explanations, when there is no direct `DEBUG(...)` path for runtime troubleshooting, or when a `debug_log(...)` wrapper hides the real source line.
...
- During existing-skill continuation or bug repair, do not modify battle runtime/framework files such as `<battle_root>/module/object/actor.lua`, `<battle_root>/module/fight/skill.lua`, `<battle_root>/module/fight/buff.lua`, `<battle_root>/module/fight/action.lua`, `<battle_root>/module/fight/damage.lua`, or `<battle_root>/module/scene/battle_scene.lua`. Treat them as read-only references unless the user explicitly asks for engine-level work. Fix the generated skill payload, task-local scripts, or the specific skill/buff/action script named by the user instead.
...
- `skill` rows must fill both `desc` and `de_desc`. Do not leave either text column blank. `skill_type` must use an existing formal-table value: `1`, `2`, `3`, `4`, `6`, or `7`; do not invent `8`.
- `skill_stage` rows follow the owning skill's maximum level
- `skill_stage` rows must use `stage=1/2/...` for the phase id. Do not write phase id `0`; the row key format remains `skill_id_stage_level`.
...
- `war_paper` rows should use Excel-side row fields directly; `name` must be a stable ASCII enum key, and custom Chinese report text must be written to `desc1`
- `war_paper` custom battle reports should be referenced in production Lua by `data_war_paper.<name>` enum names, not by passing hardcoded numeric record IDs through buff/action params
- `war_paper` payload rows should prefer stable `name` values and may omit `ID`; when `ID` is omitted, writeback must append from the current workbook's max war-report ID
- nested arrays should be serialized into Excel-friendly comma or pipe formats
- Excel config cells must never contain JSON, Python, or Lua table literals. Do not output values such as `["ATK",10000]`, `['ATK',10000]`, `{ATK=10000}`, or `{"attr":"ATK"}` inside `skill_stage.param`, `buff.param`, or any Excel parameter field. Use the real Excel parser format instead: one-dimensional values as `ATK,10000`, multiple groups as `ATK,10000|DEF,5000`, with enum tokens unquoted and without brackets.
...
Do not hardcode large guessed battle-report IDs inside skill config just to make that custom entry usable.
Use the battle-report row `name` as the stable contract and let Excel writeback allocate or reuse the numeric ID.
